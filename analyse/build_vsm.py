...
class Tf_IDf(object):
    ...
    def build_vsm(self, filename):
        ...
        print('总关键字个数: %s' % str(len(tf_idf_dict)))
...
class Build_Vsm(Tf_IDf):
    def __init__(self, time, day):
        self.tf_idf_instance = Tf_IDf(time, day)

    def build_vsm(self, contents, filename):
        """
        构建向量化空间模型，一、进行特征提取，目的是减少向量化维度
        二、进行文本相似度比较，两种实现方法。
        :param contents:
        :return:
        """
        # self.save_content_to_file(contents)
        tf_idf_dict = self.tf_idf_instance.tf_idf()
        tf_idf_list = sorted(tf_idf_dict.items(), key=lambda d: d[1], reverse=True)[:1000]
        keyword_list = [word for (word, value) in tf_idf_list if float(value) >= 0.001]
        removed_words = []
        ...
        two_content = combinations(two_score[:10], 2)
        text = combinations(contents[:10], 2)
        corpus = combinations(removed_words[:10], 2)
        for words, content, seg in zip(two_content, text, corpus):
            print(content)
            print(seg)
            number = nltk.cluster.cosine_distance(words[0], words[1])
            print('nltk相似度算法 :  {}'.format(number))
            self.get_distance(words, content)
            # with open('1234.txt', 'ab') as f:
            #      f.write(str(score).encode('utf-8') + '\n'.encode('utf-8'))
